...
export default {
  ...
  methods: {
    ...
    updateVariables() {
      if (this.payload) {
        if (this.payload[0]) {
          if (this.payload[0].vaccine_name === "JANSSEN") {
            this.checkbox = true;
            this.responses.dose_1 = this.payload[0].dose;
            this.responses.date_1 = format(
              parseISO(this.payload[0].vaccination_date),
              "MMMM dd, yyyy"
            );
            this.responses.vaccine_1 = this.payload[0].vaccine_name;
        ...
        if (this.payload[1]) {
          ...
          this.responses.date_2 = this.payload[1].vaccination_date
            ? format(
                parseISO(this.payload[1].vaccination_date),
                "MMMM dd, yyyy"
              )
            : null;
